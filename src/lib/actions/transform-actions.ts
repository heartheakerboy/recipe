...
export async function saveTransformedDraftAction(
  ...
  try {
    ...
    const updated = await recipeRepository.update(recipeId, {
      ...
      prepTimeMinutes: content.prepTimeMinutes ?? existingRecipe?.prepTimeMinutes,
      cookTimeMinutes: content.cookTimeMinutes ?? existingRecipe?.cookTimeMinutes,
      totalTimeMinutes: content.totalTimeMinutes ?? existingRecipe?.totalTimeMinutes,
